...
T_prechunk = ds_T_full.where(sel_area, drop=True).isel(time=0)
S_prechunk = ds_S_full.Salt.where(sel_area, drop=True).isel(time=0)
U_prechunk = ds_UV_full_interp.U.where(sel_area, drop=True).isel(time=0)
V_prechunk = ds_UV_full_interp.V.where(sel_area, drop=True).isel(time=0)
W_prechunk = ds_W_full_interp.W.where(sel_area, drop=True).isel(time=0)
