...
Um_lowres_MLD = Um_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Um_lowres_MLD').chunk(chunks={"time":1, "lat":len(Um_lowres.lat), "lon": len(Um_lowres.lon)})
Vm_lowres_MLD = Vm_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Vm_lowres_MLD').chunk(chunks={"time":1, "lat":len(Um_lowres.lat), "lon": len(Um_lowres.lon)})
Wm_lowres_MLD = Wm_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Wm_lowres_MLD').chunk(chunks={"time":1, "lat":len(Um_lowres.lat), "lon": len(Um_lowres.lon)})
Bm_lowres_MLD = Bm_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Bm_lowres_MLD').chunk(chunks={"time":1, "lat":len(Um_lowres.lat), "lon": len(Um_lowres.lon)})
del Um_lowres, Vm_lowres, Wm_lowres, Bm_lowres

# submeso fluxes MLD average
UsBs_lowres_MLD = UsBs_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('UsBs_lowres_MLD').chunk(chunks={"time":1, "lat":len(UsBs_lowres.lat), "lon": len(UsBs_lowres.lon)})
VsBs_lowres_MLD = VsBs_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('VsBs_lowres_MLD').chunk(chunks={"time":1, "lat":len(UsBs_lowres.lat), "lon": len(UsBs_lowres.lon)})
WsBs_lowres_MLD = WsBs_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('WsBs_lowres_MLD').chunk(chunks={"time":1, "lat":len(UsBs_lowres.lat), "lon": len(UsBs_lowres.lon)})
del UsBs_lowres, VsBs_lowres, WsBs_lowres

# meso gradients MLD average
Bm_x_lowres_MLD = Bm_x_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Bm_x_lowres_MLD').chunk(chunks={"time":1, "lat":len(Bm_x_lowres.lat), "lon": len(Bm_x_lowres.lon)})
Bm_y_lowres_MLD = Bm_y_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Bm_y_lowres_MLD').chunk(chunks={"time":1, "lat":len(Bm_x_lowres.lat), "lon": len(Bm_x_lowres.lon)})
Bm_z_lowres_MLD = Bm_z_lowres.where(delta_sigma<=0.03).mean(dim="k",skipna=True).rename('Bm_z_lowres_MLD').chunk(chunks={"time":1, "lat":len(Bm_x_lowres.lat), "lon": len(Bm_x_lowres.lon)})
