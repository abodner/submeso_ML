...
torch_dataset_train = Data.TensorDataset(
    torch.flatten(torch.from_numpy(X_input[train_ind]).float(), start_dim=1) ,
    torch.flatten(torch.from_numpy(Y_output[train_ind]).float(), start_dim=1) ,
)
...
print('TRAIN')
print('X input shape:')
print(torch.flatten(torch.from_numpy(X_input[train_ind]).float(), start_dim=1).shape)
print('Y output shape:')
print( torch.flatten(torch.from_numpy(Y_output[train_ind]).float(), start_dim=1).shape)
print('')

###### test dataset #######
torch_dataset_test = Data.TensorDataset(
    torch.flatten(torch.from_numpy(X_input[test_ind]).float(), start_dim=1),
    torch.flatten(torch.from_numpy(Y_output[test_ind]).float(), start_dim=1)    
)
...
print('TEST')
print('X input shape:')
print(torch.flatten(torch.from_numpy(X_input[test_ind]).float(), start_dim=1).shape)
print('Y output shape:')
print( torch.flatten(torch.from_numpy(Y_output[test_ind]).float(), start_dim=1).shape)
print('')
...
def train():
   ...
   # figure_power=wandb.Image(figure_power)
    ...
    r2_avg = torch.tensor(r2_list).mean().item()
    corr_avg = torch.tensor(corr_list).mean().item()
...
sweep_configuration = {
    ...
    'parameters': {
        'num_hidden_layers': {'values': [2,3,4,5,6,7,8,9,10]},
        'neurons_per_layer': {'values': [16,32, 64, 128]},
