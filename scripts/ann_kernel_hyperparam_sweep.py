...
def img2vec(img, kernel_size=3):
    shift = int(kernel_size/2)
    vec = []
    for j in range(shift,img.shape[-2]-shift):
        for i in range(shift,img.shape[-1]-shift):
            vec.append(img[:,0,j-shift:j+shift+1,i-shift:i+shift+1].reshape((img.shape[0],kernel_size**2)))
...
torch_dataset_train = Data.TensorDataset(
    img2vec(torch.from_numpy(X_input[train_ind])).float(),
    img2vec(torch.from_numpy(Y_output[train_ind])).float(),
)
...
print('TRAIN')
print('X input shape:')
print(img2vec(torch.from_numpy(X_input[train_ind])).float().shape)
print('Y output shape:')
print( img2vec(torch.from_numpy(Y_output[train_ind])).float().shape)
print('')

###### test dataset #######
torch_dataset_test = Data.TensorDataset(
    img2vec(torch.from_numpy(X_input[test_ind])).float(),
    img2vec(torch.from_numpy(Y_output[test_ind])).float(),   
)
...
print('TEST')
print('X input shape:')
print(img2vec(torch.from_numpy(X_input[test_ind])).float().shape)
print('Y output shape:')
print( img2vec(torch.from_numpy(Y_output[test_ind])).float().shape)
print('')
...
sweep_configuration = {
    ...
    'parameters': {
        ...
        'neurons_per_layer': {'values': [16,32, 64]},
